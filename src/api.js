...
module.exports = function(ctx, api) {
    ...
    api.add = function(geojson) {
        const errors = geojsonhint.hint(geojson, { precisionWarning: false }).filter(function(e) { return e.level !== 'message' });
        if (errors.length) {
    ...
    api.setFeatureCoordinates = function(featureId, path, lng, lat) {
        ctx.store.setFeatureCoordinates(featureId, path, lng, lat)
        return api;
    };
